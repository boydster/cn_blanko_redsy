		...
		if ($context['show_bbc'] && $bbcContainer !== null)
		{
						...
						echo '
							{
								...
								sImage: ', isset($tag['fa-icon']) ? JavaScriptEscape($tag['fa-icon']) : JavaScriptEscape($settings['images_url'] . '/bbc/' . $tag['image'] . '.gif'), ',
			...
			echo '
					...
					sButtonTemplate: ', JavaScriptEscape('
						<span class="btn btn-danger btn-sm bbc_button" id="%buttonId%" title="%buttonDescription%"><i class="fa %buttonSrc% bbc_glyph"></i></span>
					'), ',
					sDividerTemplate: ', JavaScriptEscape('
						<img class="bbc_divider" src="' . $settings['images_url'] . '/bbc/divider.gif" alt="|" style="margin: 0 3px 0 3px;" />
					'), ',
					sSelectTemplate: ', JavaScriptEscape('
						<br class="mobile_only %selectName%_br">
						<select name="%selectName%" class="%selectName%" id="%selectId%" style="margin-bottom: 1ex; font-size: x-small;">
							%selectOptions%
						</select>
					'), ',
